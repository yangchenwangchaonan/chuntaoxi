...
Page({
  ...
  loadProductDetail: function () {
    ...
    wx.request({
      url: app.d.ceshiUrl + '/Api/GroupBuy/index',
      method: 'post',
      data: {
        id: that.data.id,
      ...
      success: function (res) {
        ...
        if (status == 1) {
          ...
          //console.log('group_price:::'+group_price);
  ...
  addShopCart: function (e) {
    ...
    console.log('pid:' + that.data.productId);
    console.log('attr:' + attrstr);
    var is_group_buy = 0;
    if (ptype =='buynow'){
      is_group_buy = 1;
    } else if (ptype == 'groupsinglebuy'){
      is_group_buy = 0;
    }
    wx.request({
      url: app.d.ceshiUrl + '/Api/Shopping/add_group',
      method: 'post',
      data: {
        uid: app.d.userId,
        pid: that.data.productId,
      ...
      success: function (res) {
        ...
        if (data.status == 1) {
          if (ptype == 'buynow') {
            wx.redirectTo({
              url: '../groupbuy/groupbuypay?cartId=' + data.cart_id
            });
            return;
          } else if (ptype == 'groupsinglebuy') {
            wx.redirectTo({
